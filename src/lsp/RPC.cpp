...
std::string EncodeMessage(const json &msg) {
  try {
    ...
    throw std::runtime_error(std::string("Failed to encode message: ") +
                             e.what());
  }
}
...
DecodeMessage(const std::vector<uint8_t> &msg) {
  ...
  auto separatorPos =
      std::search(msg.begin(), msg.end(), separator.begin(), separator.end());
  ...
  try {
    contentLength =
        std::stoi(header.substr(contentLengthPos + contentLengthPrefix.size()));
  ...
  try {
    jsonContent =
        nlohmann::json::parse(content.begin(), content.begin() + contentLength);
  } catch (const std::exception &e) {
  ...
  return {
      baseMessage.method,
      std::vector<uint8_t>(content.begin(), content.begin() + contentLength)};
}
