...
std::pair<int, std::string> LSPState::getWordAtCursor(const std::string& content, int line, int character) {
    ...
    int start = character;
    while (start > 0 && (std::isalnum(targetLine[start - 1]) || targetLine[start - 1] == '_')) {
        start--;
    }
...
HoverResponse LSPState::hover(HoverRequest request) {
    ...
    auto [start, token] = getWordAtCursor(contents, position.line, position.character);
    ...
    if (node) {
        ...
        if (getVariableNode.isFunction()) {
            auto fnType = analysisResults->getTypeResults()->getInferredType(node.value());
            ...
            auto varType = analysisResults->getTypeResults()->getInferredType(varNode);
    ...
    //             auto fnType = analysisResults->getTypeResults()->getInferredType(fn);
    //             std::stringstream typeName;
    //             typeName << token << ": " << *fnType;
    //             return newHoverResponse(request.id, typeName.str());
    //         }
    ...
    //             if (local->getLine() - 1 == position.line && local->getColumn() == start) {
    //                 auto localType = analysisResults->getTypeResults()->getInferredType(local);
    //                 std::stringstream typeName;
